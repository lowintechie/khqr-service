...
export class KhqrController {
  ...
  async createKhqr(@Body() param: CreateKhqrDto): Promise<KhqrResponse> {
    this.logger.log(`Creating KHQR for type: ${param.type}`);

    try {
      const result = await this.khqrService.getKhqr(param.type, param.data);
  ...
  async checkTransaction(@Body() param: CheckTransactionDto): Promise<object> {
    this.logger.log(`Checking transaction with MD5: ${param.md5}`);
    try {
      return await checkBakongTransaction(param.bakongToken, param.md5);
    } catch (error) {
  ...
  @Post('deeplink')
  async generateDeeplinkEndpoint(
    @Body() body: GenerateDeeplinkDto,
